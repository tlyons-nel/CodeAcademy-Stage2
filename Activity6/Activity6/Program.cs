...
namespace Activity5
{
    static class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"Contact {maxContact.FirstName} {maxContact.LastName} has the most businesses associated with { maxContactBusinessCount } ");

            Console.WriteLine("\n***********************************************");
            Console.WriteLine("* Activity #6 - Step 1                        *");
            Console.WriteLine("***********************************************\n");

            var businesses1 = Businesses(contacts[0], businesses);
            maxContact = null;
            maxContactBusinessCount = 0;

            foreach (var contact in contacts)
            {
                var count = contact.Businesses(businesses).Length;
                if (maxContact == null || count > maxContactBusinessCount)
                {
                    maxContact = contact;
                    maxContactBusinessCount = count;
            ...
            Console.WriteLine("\n***********************************************");
            Console.WriteLine("* Activity #6 - Step 2                        *");
            Console.WriteLine("***********************************************\n");

            var gabieBusinesses2 = businesses.Search("Gabie", "Boulder");
          
            foreach (var business in gabieBusinesses2)
            {
                Console.WriteLine($"Gabie Boulder is a contact for Business {business.Name}");
            }

            //*********************************************
            Contact cc = null;
            Console.WriteLine(cc.NullSomething());
        }
        ...
        static int BusinessesAssociatedWithContact(int contactID, Business[] businesses)
        {
            return businesses.Where(b => b.Contacts.Any(c => c.Id == contactID)).Count();
        }

        // Extension methods - Activity #6
        ...
        {
            return businesses.Where(b => b.Contacts.Any(c => c.FirstName == firstName && c.LastName == lastName)).ToArray();
        }

        public static string NullSomething(this Contact contact)
        {
